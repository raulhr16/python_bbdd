import cx_Oracle
import sys
cx_Oracle.init_oracle_client(lib_dir=r"C:\oraclexe\instantclient\instantclient_21_13")

try:
    db = cx_Oracle.connect(user='proyectopython', password='raulhr', dsn='localhost:1521/XE')
    cursor = db.cursor()
except cx_Oracle.DatabaseError as e:
    error, = e.args
    print("No puedo conectar a la base de datos:", error)
    sys.exit(1)
...
def ejlistar(db):
	...
	try:
		cursor.execute(sql)
		centrales=len(cursor.fetchall())
		cursor.execute(sql_hidro)
		hidro = len(cursor.fetchall())
		cursor.execute(sql_solar)
		solar = len(cursor.fetchall())
		cursor.execute(sql_termica)
		termica = len(cursor.fetchall())
		cursor.execute(sql_eolicas)
		eolicas = len(cursor.fetchall())
	...
	try:
		...
		for registro in registros:
			if registro[2]==nom_empresa:
				...
				if registro[3]==nom_propietario:
					validacion2=2
					cif_empresa=registro[0]
					centrales=cursor_centrales.fetchall()
					for central in centrales:
						if cif_empresa==central[1]:
							print("-"*65)
							print("Código de las centrales"," "*5+"Ubicación"," "*5+"Capacidad Máxima")
							print(central[0]," "*20,central[3]," "*(13-len(central[3])),central[4])
	except:
		print("Error en la consulta")
	if validacion==0:
